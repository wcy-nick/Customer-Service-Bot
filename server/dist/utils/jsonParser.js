...
exports.extractTextFromJSON = extractTextFromJSON;
exports.extractStructuredTextFromJSON = extractStructuredTextFromJSON;
...
function extractTextFromJSON(filePath) {
    try {
        ...
        if (data.deltas && typeof data.deltas === 'object') {
            // 遍历所有delta区域
            Object.values(data.deltas).forEach((delta) => {
 ...
 * @returns 提取的结构化文本内容
 */
function extractStructuredTextFromJSON(filePath) {
    try {
        const jsonContent = fs_1.default.readFileSync(filePath, 'utf-8');
        const data = JSON.parse(jsonContent);
        // 存储提取的结构化文本
